 ...
 */
const setFirstWeekBlankLine = (dayWeek: number, calData: CalDay[]): void => {
 ...
 * @param {Array.<Object>} calData
 */
const setLastBlankLine = (calData: CalDay[]): void => {
  for (let i = 0; i < 7; i++) {
 ...
 * @return {boolean}
 */
const checkToday = (year: number, month: number, day: number): boolean => {
...
export class SwsCalendar {
  ...
  set(year: number, month: number): SwsCalendar {
    ...
    for (let i = 0; i < numDays; i++) {
      ...
      const calDay = CalDay.createNew(dayWeek, Math.trunc((firstDayWeek + day - 1) / 7), options);
